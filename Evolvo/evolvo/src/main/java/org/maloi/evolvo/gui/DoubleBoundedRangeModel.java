...
/**
 * This class is an extension to DefaultBoundedRangeModel which
 * allows the range to be over double-sized floating point numbers.
 * In order to maintain the standard interface of BoundedRangeModel,
 * this class stores numbers only to a desired precision.  When
 * one of the methods defined by the BoundedRangeModel interface
 * are called, they return their expected value multiplied by
 * 10^precision.  There are also accessor methods to set and retrieve
 * the actual floating point numbers.
 * I chose to extend DefaultBoundedRangeModel rather than
 * implementing BoundedRangeModel to avoid rewriting code for the
 * event handlers.  No need to reinvent the wheel... 
 *
 */
class DoubleBoundedRangeModel extends DefaultBoundedRangeModel
{
   private static final long serialVersionUID = 8460344160737262670L;
