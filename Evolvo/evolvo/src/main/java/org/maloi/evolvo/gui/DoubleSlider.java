 ...
 *
 * This actually uses a little trickery so that the JSlider class didn't have 
 * to be rewritten.  All values are actually stored as integers, and 
 * multiplied or divided by a multiplier to translate from the underlaying 
 * Slider's methods that deal with integers and this objects methods that deal
 * with doubles.  The multiplier is a power of 10, and determines the accuracy
 * of the slider.  
 */
public class DoubleSlider extends JSlider
{
   private static final long serialVersionUID = -4077077078430984640L;
   ...
   /** Class constructor that takes all parameters but precision, 
    *  defaulting to a precision of 2 decimal places. 
    */
      ...
      int precision)
   {
      ...
      DoubleBoundedRangeModel dbrm =
         new DoubleBoundedRangeModel(v, e, minimum, maximum, precision);
   ...
   {
      DoubleBoundedRangeModel dbrm = (DoubleBoundedRangeModel) this.getModel();
      return (double) super.getMajorTickSpacing()
         / (double) dbrm.getMultiplier();
   }
   ...
   {
      DoubleBoundedRangeModel dbrm = (DoubleBoundedRangeModel) this.getModel();
      return (double) super.getMinorTickSpacing()
         / (double) dbrm.getMultiplier();
   }
   ...
   {
      var translatedLabels = new Hashtable<Integer, JComponent>();
      Double oldValue;
      Integer newValue;
      ...
      for (Enumeration e = labels.keys(); e.hasMoreElements();)
      {
         ...
         newValue =
            Integer.valueOf(
               (int) (oldValue.doubleValue()
                  * ((DoubleBoundedRangeModel) (this.getModel()))
                     .getMultiplier()));
         translatedLabels.put(newValue, (JComponent)labels.get(oldValue));
      }
